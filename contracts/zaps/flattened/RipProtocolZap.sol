...
contract RipProtocolZap is ZapBaseV2_2 {
    ...
    modifier onlyRipProtocolDAO {
        require (msg.sender == ripprotocolDAO);
        _;
    }
    ...
    function ZapIn(
        ...
        bool bond
    ) external payable stopInEmergency returns ( uint RIPRec ) {
        if ( bond ) {
    ...
    function removeLiquidityReturn(
        address fromToken, 
        uint256 fromAmount
    ) external view returns (uint256 ripAmount) {
        if (fromToken == sRIP) {
            return fromAmount;
    ...
    function update_RipProtocolDAO(
        address _ripprotocolDAO
    ...
    function update_sRIP(
        address _sRIP
    ) external onlyRipProtocolDAO {
       sRIP = _sRIP;
    }

    function update_wsRIP(
        address _wsRIP
    ) external onlyRipProtocolDAO {
        wsRIP = _wsRIP;
    }
